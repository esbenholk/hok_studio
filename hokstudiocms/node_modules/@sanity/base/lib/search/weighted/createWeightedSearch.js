...
function createWeightedSearch(types, client) {
  ...
  return function search(queryString) {
    ...
    var filters = ['_type in $__types', includeDrafts === false && "!(_id in path('drafts.**'))", ...constraints.map(constraint => "(".concat(constraint.join('||'), ")")), filter ? "(".concat(filter, ")") : ''].filter(Boolean);
    ...
    return client.observable.fetch(query, _objectSpread(_objectSpread({}, toGroqParams(terms)), {}, {
      ...
      __limit: 1000
    }, params || {})).pipe((0, _operators.map)(_draftUtils.removeDupes), (0, _operators.map)(hits => (0, _applyWeights.applyWeights)(searchSpec, hits, terms)), (0, _operators.map)(hits => (0, _sortBy2.default)(hits, hit => -hit.score)), (0, _operators.map)(hits => hits.slice(0, 100)));
  };
}