...
export default function useMaterial(props) {
  ...
  const [gloss, noise] = useTexture(["/gloss_map.jpg", "/noise.png"]);
  ...
  const pipeline = useMemo(() => {
    ...
    const transmissionMaterial = new ShaderMaterial({
      ...
      uniforms: {
        ...
        transmissionIntensity: {
          value: _transmissionMaterial.transmissionIntensity,
        },
        distortionIntensity: {
          value: _transmissionMaterial.distortionIntensity,
        },
    ...
    const lambda1 = new LambdaPass(
      () => void (ref.current.material = frontMaterial)
    );
    const lambda2 = new LambdaPass(
      () => void (ref.current.material = backMaterial)
    );
